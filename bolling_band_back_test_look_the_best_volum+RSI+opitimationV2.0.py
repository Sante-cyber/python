                ...
                if i<len(df)-1:
                    ...
                    if not pre_row.empty :
                        if is_trade==0 and data.signal=='buy' and data.buy_cnt==1:
                            is_trade=1
                            trade_signal='buy'
                        elif is_trade==0 and data.signal=='sell' and data.sell_cnt==1:
                            is_trade=2
                            trade_signal='sell'
                    if trade==True:
                        # print(is_trade)
                        if is_trade==1  and self.trading_allowed():
                            # print(data.time,data.buy_cnt,pre_row.buy_cnt)
                            if data.buy_cnt==0 and pre_row.buy_cnt==1 and (pre_row.rsi+pre_row.low_rsi)/2<30 and data.lower_30==0:
