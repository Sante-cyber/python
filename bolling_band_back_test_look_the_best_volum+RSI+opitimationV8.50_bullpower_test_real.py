              ...
              if i>1:
                ...
                if i<len(df)-1:
                    ...
                    if trade==True:
                        ...
                        if is_trade==1.1  and self.trading_allowed():
                            ...
                            if  pre_row.buy_cnt==2 and data.buy_cnt==0 and data.lower_30==0 \
                                ...
                                if next_row.low<=order_price:
                                 ...
                                 and pre_row.high_rsi<pre_row.low_rsi and data.high_rsi>data.low_rsi\
                                 and pre_2_row.low_rsi<30 and pre_row.low_rsi>max(pre_row.rsi,pre_row.high_rsi) and pre_2_row.high_rsi<30\
                                 and pre_2_row.high_rsi>pre_2_row.low_rsi:
                                order_price=data.close
                                if next_row.low<=order_price:
                                    sl=order_price-0.006*order_price
                                    tp=order_price+0.006*order_price
                            ...
                            if pre_row.buy_cnt>0 and data.buy_cnt==0 and data.lower_30==0:
                                ...
                                is_trade=1.61
                        elif is_trade==1.61 and self.trading_allowed():
                        ...
                        elif is_trade==3.41 and self.trading_allowed():                            
                                order_price=pre_row.close
                                if data.high>=order_price:
                                    sl=order_price+0.006*order_price
                                    tp=order_price-0.005*order_price   
                        ...
                        elif is_trade==3.42 and self.trading_allowed():                            
                                order_price=pre_row.close
                                if data.high>=order_price:
                                    sl=order_price+0.007*order_price
                                    tp=order_price-0.007*order_price   
                        ...
                        elif is_trade==3.43 and self.trading_allowed():                            
                                order_price=pre_row.close
                                if data.high>=order_price:
                                    sl=order_price+0.007*order_price
                                    tp=order_price-0.006*order_price   
                        ...
                        elif is_trade==3.44 and self.trading_allowed():                            
                                order_price=pre_row.close
                                if data.high>=order_price:
                                    if pre_row.sd>0.009:
                                        sl=order_price+0.01*order_price
...
for year in years:
    ...
    for currency in symbol:
        ...
        bars_d=mt.copy_rates_range(currency,mt.TIMEFRAME_D1,datetime(year-7,1,1), datetime(year,12,31))
        ...
        df1['time']=pd.to_datetime(df['time'],unit='s')
        ...
        df["BullsPower"] = df["high"] - df["EMA"]
        df["BearsPower"] = df["low"] - df["EMA"]
        df["EMA_slope_up"] = df["EMA"].diff() > 0
        df["EMA_slope_down"] = df["EMA"].diff() < 0

        df1["EMA_D1"] =df1["close"].ewm(span=EMA_PERIOD, adjust=False).mean()
        df1["D1_up"] = df1["EMA_D1"].diff() > 0
        df1["D1_down"] = df1["EMA_D1"].diff() < 0

        htf = df1[["EMA_D1","D1_up","D1_down"]].copy()
        htf = df.join(htf, how="left").ffill()[["EMA_D1","D1_up","D1_down"]]
        df[["EMA_D1","D1_up","D1_down"]] = htf

        USE_HTF_TREND = True
        
        df["bull_type"] = np.where(
            (df["EMA_slope_up"]) 
            & (df["BearsPower"] < 0) 
            & (df["BearsPower"] > df["BearsPower"].shift(1)) 
            & ((~USE_HTF_TREND) | (df["D1_up"])),   # require D1_up if filter ON
            1,   # long

            np.where(
                (df["EMA_slope_down"]) 
                & (df["BullsPower"] > 0) 
                & (df["BullsPower"] < df["BullsPower"].shift(1)) 
                & ((~USE_HTF_TREND) | (df["D1_down"])),  # require D1_down if filter ON
                -1,  # short
                0    # no signal
            )
        )
