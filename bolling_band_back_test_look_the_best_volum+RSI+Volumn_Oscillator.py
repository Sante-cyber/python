...
#  datetime.now()
        ...
        # for short in range(1,21+3,3):
        #   for long in range(20,51+2,5):
        ...
        df['short_volume_ma'] = df['tick_volume'].rolling(window=19).mean()
        df['long_volume_ma'] = df['tick_volume'].rolling(window=30).mean()
        df['volume_oscillator'] = df['short_volume_ma'] - df['long_volume_ma']
    ...
    # fig.show()
        ...
        df=rsi(df,14)
        df['signal']=np.vectorize(find_signal)(df['close'],df['lb'],df['ub'],df['rsi'],df['overbought'],df['oversold'],df['volume_oscillator'])
