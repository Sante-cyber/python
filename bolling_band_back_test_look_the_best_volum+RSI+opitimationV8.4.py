                ...
                if i<len(df)-1:
                    ...
                    if not pre_row.empty :
                        if is_trade==0 \
                            ...
                            trade_signal='buy'
                        # elif is_trade==0 \
                        #         and pre_row.rsi>29 and pre_row.rsi<30 and data.rsi<68 and pre_row.close>pre_row.ub:
                        #     is_trade=2.6
                        #     trade_signal='sell'
                        elif is_trade==0 \
                    ...
                    if trade==True:
                        ...
                        if is_trade==1.1   and self.trading_allowed():
                            ...
                            if (pre_row.rsi>=28 and data.low_rsi<30 and data.high_rsi<30) or (pre_row.rsi<28):
                                order_price=data.close
                                if next_row.low<=order_price:
                                    sl=order_price-2*data.sd
                                    tp=order_price+2*data.sd
                            ...
                            #         self.add_position(position(next_row.time,order_price,trade_signal,self.volume,sl,tp,currency,is_trade))
                            #         is_trade=0
                            #     else:is_trade=0 (delete this code)
                            ...
                            if track_point<=1:
                                ...
                                #       is_trade=0  
                                #       track_point=0   
                        elif is_trade==1.9   and self.trading_allowed():
                        ...
                        elif is_trade==1.92   and self.trading_allowed():
                            if pre_row.low_rsi>pre_row.high_rsi and data.high_rsi>30:
                                order_price=data.close
                                if next_row.low<=order_price:
                                    sl=order_price-2*data.sd
                                    tp=order_price+2*data.sd
                            ...
                            elif data.over_70==0 and pre_2_row.over_70>=2 and pre_2_row.over_70<=3 and pre_row.over_70==0 and abs((pre_row.close-pre_row.open)/pre_row.close)>=0.00001:
                                order_price=data.close
                                if next_row.high>=order_price:
                                    sl=order_price+2*data.sd
                                    tp=order_price-2*data.sd
                          ...
                          if pre_row.sell_cnt>0 and data.sell_cnt==0 and pre_row.rsi<80 and pre_row.high_rsi>80:
                              ...
                              is_trade=0 
                        elif is_trade==2.7 and self.trading_allowed():
...
#  datetime.now()
        ...
        # df.to_csv(f'E:/EA/bollinger-bands/H4_year/b_{year}_opi_8.4.csv')
        df.to_csv(f'C:/c/EA/bollinger-bands/H4_year/b_{year}_opi_8.4.csv')
        ...
        df.to_csv(f'C:/c/EA/bollinger-bands/H4_year/b_{year}_opi_result_8.4.csv',index=False)
        # df.to_csv(f'E:/EA/bollinger-bands/H4_year/b_{year}_opi_result_8.4.csv',index=False)
...
df1.to_csv(f'C:/c/EA/bollinger-bands/H4_year/result_detail_volumn_rsi_opi_8.4.csv')
df2.to_csv(f'C:/c/EA/bollinger-bands/H4_year/final_result_volumn_detail_rsi_opi_8.4.csv')
# df1.to_csv(f'E:/EA/bollinger-bands/H4_year/result_detail_volumn_rsi_opiti_8.4.csv')
# df2.to_csv(f'E:/EA/bollinger-bands/H4_year/final_result_volumn_detail_opiti_8.4.csv')
