...
def find_signal(open,session_high,session_low,gmt_hour,rsi,ma):
    if open < session_low and gmt_hour >= 8 and gmt_hour < 12 and rsi < 30 and open < ma:
        return 'sell'
    elif open > session_high and gmt_hour >= 8 and gmt_hour < 12 and rsi > 70 and open > ma:
        return 'buy'
            ...
            if i>=1:
                ...
                if i<len(self.data)-1:
                    ...
                    if not pre_row.empty :
                        ...
                        # print(data.buy_cnt)
                        # print(is_trade)
                        if is_trade==0 and pre_row.buy_cnt==1 and data.buy_cnt>0 and 12>data.gmt_hour>=8:
                        ...
                        if is_trade==1.0 and self.trading_allowed() :
                          if data.buy_cnt==0:
                            ...
                            if data.low<=order_price:
                                ...
                                sl,tp=calculate_sl_tp(order_price,pre_row.session_high_y, pre_row.session_low_y, pre_row.atr,pre_row.signal)
...
for year in years:
    ...
    for currency in symbol:
        ...
        df['signal']=np.vectorize(find_signal)(df['open'],df['session_high_y'],df['session_low_y'],df['gmt_hour'],df['rsi'],df['ma'])
